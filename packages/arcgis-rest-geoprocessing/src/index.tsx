...
export const createJob = (url: string, monitorEvents: boolean, monitorEventRate: number, params: {}, authToken: string) => {
  ...
  const response = fetch(cleanUrl(submitJobUrlCheck), options).then(response => response.json()).then((results: { jobId: string, jobStatus: string}) => results);

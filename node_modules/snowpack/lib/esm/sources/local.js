...
import { resolveDependencyManifest as _resolveDependencyManifest, resolveEntrypoint, } from 'esinstall';
...
import { createInstallTarget, findMatchingAliasEntry, getExtension, isJavaScript, isPathImport, isRemoteUrl, parsePackageImportSpecifier, readFile, } from '../util';
...
export class PackageSourceLocal {
    constructor(config) {
    ...
    async setupCacheDirectory() {
        ...
        if (config.dependencies) {
            await fs.writeFile(path.join(packageSourceDirectory, 'package.json'), JSON.stringify({
            ...
            if (existsSync(lockfileLoc)) {
                ...
                if (existsSync(manifestLockLoc))
                    await fs.unlink(manifestLockLoc);
            }
        }
        else {
            if (existsSync(manifestLoc))
                await fs.unlink(manifestLoc);
            if (existsSync(manifestLockLoc))
                await fs.unlink(manifestLockLoc);
        }
    }
    ...
    async prepare() {
        ...
        if (installDirectoryHash === CURRENT_META_FILE_CONTENTS) {
            ...
            logger.info('Snowpack updated! Rebuilding your dependencies for the latest version of Snowpack...');
            await this.clearCache();
        }
        else {
            logger.info(`${colors.bold('Welcome to Snowpack!')} Because this is your first time running\n` +
    ...
    async load(id, { isSSR } = {}) {
        ...
        const packageImportMap = JSON.parse(await fs.readFile(path.join(installDest, 'import-map.json'), 'utf8'));
        ...
        packageCode = await transformFileImports({ type, contents: packageCode }, async (spec) => {
            ...
            imports.push(createInstallTarget(path.resolve(path.posix.join(config.buildOptions.metaUrlPath, 'pkg', id), resolvedImportUrl)));
    ...
    async resolvePackageImport(_spec, options = {}) {
        ...
        if (isSymlink && config.workspaceRoot && isWithinRoot) {
            ...
            logger.warn(colors.bold(`${spec}: Locally linked package detected outside of project root.\n`) +
        ...
        if (options.importMap) {
            if (options.importMap.imports[spec]) {
                return path.posix.join(config.buildOptions.metaUrlPath, 'pkg', options.importMap.imports[spec]);
            }
        ...
        logger.warn(colors.bold(`${spec}: Unscannable package import found.\n`) +
